...
public class ExpenseController {
  ...
  public ResponseEntity<List<ExpenseResponse>> getExpenseByMonthAndCurrentUser(
      @RequestParam String month) {
    return ResponseEntity.ok(expenseService.findExpenseByMonthAndCurrentUser(month));
  }

  @GetMapping
  public ResponseEntity<List<ExpenseResponse>> getExpensesByCurrentUser() {
    return ResponseEntity.ok(expenseService.findExpensesByCurrentUser());
  }

  @GetMapping("/{id}")
  public ResponseEntity<ExpenseResponse> getExpenseByIdAndCurrentUser(
      @PathVariable("id") Long userId) {
    return ResponseEntity.ok(expenseService.findExpenseByIdAndCurrentUser(userId));
  }

  @PutMapping("/{id}")
  public ResponseEntity<ExpenseResponse> updateExpense(
      @PathVariable("id") Long expenseId, @RequestBody @Valid ExpenseRequest expenseRequest) {
